...
!
      ...
      real(r8) :: kg2mmolC = 1000.5_r8/12.0107_r8
      real(r8) :: r_om=0.15_r8
      real(r8) :: cff9
      integer :: ised
#endif
...
#endif
#if defined ODU && defined OXYGEN
!
! Assume ODUs react with oxygen instantaneously
!
                cff1=MIN(BIO(i,k,iOxyg),Bio(i,k,iODU_))
                Bio(i,k,iOxyg)=Bio(i,k,iOxyg)-cff1
                Bio(i,k,iODU_)=Bio(i,k,iODU_)-cff1
#endif
!
...
# if defined SEDBIO_COUP
              DO itrc=1,NST
                IF (sed_rxn(itrc,ng).gt.0.0_r8) THEN
                  cff9=kg2mmolC*r_om
                  ised=idsed(itrc)
! For now, assume water column POM remineralizes at same rate as 
! large detritus
                  N_Flux_RemineL=N_Flux_RemineL +                       &
     &               Bio(i,k,ised)*cff9*cff3
                  Bio(i,k,ised)=Bio(i,k,ised)*cff4
                END IF
              END DO
# endif
...
# if defined SEDBIO_COUP
              DO itrc=1,NST
                IF (sed_rxn(itrc,ng).gt.0.0_r8) THEN
                  cff9=kg2mmolC*r_om
                  ised=idsed(itrc)
!For now, assume POM remineralizes at same rate as large detritus
                  N_Flux_RemineL=N_Flux_RemineL +                       &
     &               Bio(i,k,ised)*cff9*cff3
                  Bio(i,k,ised)=Bio(i,k,ised)*cff4
                END IF
...
#if defined BIO_SEDIMENT && !defined SEDBIO_COUP
!
...
#  endif
     ...
     &          (ibio.eq.iLDeC))THEN
            ...
            END IF
# endif
#elif defined SEDBIO_COUP
!
! Particulate flux of phytoplankton and detritus reaching the seafloor
! is incorporated into sediment-POM variables
!
...
!               cff1=FC(i,0)*Hz_inv(i,1)
                cff1=FC(i,0)
# ifdef RHONE
! Assume POM contributes to last sediment class
                bed_mass(i,j,1,nstp,NST  )=bed_mass(i,j,1,nstp,NST  )+  &
     &                 0.5*cff1
                bed_mass(i,j,1,nstp,NST-1)=bed_mass(i,j,1,nstp,NST-1)+  &
     &                 0.5*cff1
# else
! Assume POM contributes to last sediment class
                bed_mass(i,j,1,nstp,NST)=bed_mass(i,j,1,nstp,NST)+cff1
# endif
              END DO
            ENDIF
#endif
...
#if defined SEDBIO_COUP
!
! Recalculate seabed thicknes, etc. for first seabed layer
!
        DO i=Istr,Iend
          cff2=0.0_r8
          DO itrc=1,NST
!            cff2=cff2+bed_mass(i,j,1,nstp,idsed(itrc))
            cff2=cff2+bed_mass(i,j,1,nstp,itrc)
          END DO
          cff2=MAX(eps,cff2)
!          IF (cff2.lt.eps) cff2=eps
          bed(i,j,1,ithck)=0.0_r8
          DO itrc=1,NST
            bed_frac(i,j,1,itrc)=bed_mass(i,j,1,nstp,itrc)/cff2
            bed(i,j,1,ithck)=bed(i,j,1,ithck)+                          &
     &         bed_mass(i,j,1,nstp,itrc)/(Srho(itrc,ng)*                &
